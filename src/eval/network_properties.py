...
def net_properties(network_file: str, wt_attr_name: str = 'wt',
                   max_edges: int = None, reverse_order:bool = False) -> List[str]:
...
def graph_properties(network_files: str, wt_attr_name: str,
                     max_edges: int, reverse_order:bool, out_file) -> None:
    cnames = ["Nodes", "Edges", "Density", "CC", "Avg.Degree"]
    prop_data = {fx : net_properties(fx, wt_attr_name, max_edges, reverse_order) for fx in network_files}
...
def net_node_properties(network_file: str, probe_map, wt_attr_name: str = 'wt',
    ...
    rev_net: nx.Graph = nx.from_pandas_edgelist(net_df, edge_attr=wt_attr_name)
    net_betw = nx.betweenness_centrality(rev_net)
    net_deg = {x: nx.degree(rev_net, x) for x in rev_net}
    net_props = {
...
def graph_node_properties(network_files: str, wt_attr_name: str, max_edges: int,
                          reverse_order:bool, probe_file: str, out_file) -> None:
...
if __name__ == "__main__":
    ...
    ARGPARSER.add_argument("-x", "--max_edges", type=int, default=None,
    ...
    if CMDARGS.node_prop is True:
        graph_node_properties(CMDARGS.network_file, CMDARGS.wt_attr, CMDARGS.max_edges,
                              CMDARGS.reverse_order, CMDARGS.probe_map_file, CMDARGS.output_file)
    else:
        graph_properties(CMDARGS.network_file, CMDARGS.wt_attr, CMDARGS.max_edges,
                         CMDARGS.reverse_order, CMDARGS.output_file)
