...
def compute_tps(annot_file, gs_file, net_file,
    ...
    tp_edges = collections.defaultdict(float)
        ...
        if udx in rv_net_graph and vdx in rv_net_graph:
            try:
                if nx.shortest_path_length(rv_net_graph, udx, vdx) == 1:
                    uve = (udx, vdx) if udx < vdx else (vdx, udx)
                    tp_edges[uve] = rv_net_graph[udx][vdx][wt_attr]
            except nx.NetworkXNoPath:
                pass
    return tp_edges

def find_tp_weights(annot_file, gs_file, network_files, network_names,
                    wt_attr, max_edges, reverse_order, tp_out_file):
    all_net_tps = [compute_tps(annot_file, gs_file, net_file,
                               wt_attr, mxe, reverse_order)
                    for net_file, mxe in zip(network_files, max_edges)]
    all_tps = set([])
    for net_tps in all_net_tps:
        all_tps = all_tps | set(net_tps.keys())
    net_weights = [float(len(net_tps))/float(len(all_tps)) for net_tps in all_net_tps]
    print("""
        Network TPS : """,  [len(net_tps) for net_tps in all_net_tps],
                            len(all_tps),
        """
        Network weights: """, net_weights)
    if tp_out_file is not None:
        all_edges = [x for x in all_tps]
        all_rcds = [ [uve[0], uve[1]] + [net_tps[uve] for net_tps in all_net_tps] 
                     for uve in all_edges]
        tp_df = pd.DataFrame(all_rcds, columns = ['SRC', 'TGT'] + network_names)
        tp_df.to_csv(tp_out_file, sep='\t', index=False)
    return net_weights


def weighted_rank_network(network_files, network_names=None, network_weights=None,
                          wt_attr: str = 'wt', max_edges: List[int] = None,
    ...
    if max_edges is None:
        max_edges = [None for _ in range(len(network_files))]
    cmb_network = pd.DataFrame(columns=['source', 'target'])
    for nx_name, nx_file, mxe in zip(network_names, network_files, max_edges):
    ...
    mx_rank = max([np.nanmax(cmb_network[nx_name]) for nx_name in network_names])
    for nx_wt, nx_name in zip(network_weights, network_names):
        if None in max_edges:
            mx_rank = max(max_edges)
        print(mx_rank)
...
def main(network_files: List[str], network_names: str,
         network_weights: str, wt_attr: str, max_edges: str,
         max_out_edges: int, max_avg: int, 
         reverse_order: bool, annotation_file: str, gs_network_file: str, 
         out_file: str, tp_out_file: str = None) -> bool:
    if network_names:
    ...
    if max_edges is not None:
        if "," in max_edges:
            max_edges = max_edges.split(",")
            if len(max_edges) != len(network_files):
                print("Length of maximum edges should be equal to length of network files")
                return False
            max_edges = [int(mxe) for mxe in max_edges]
        else:
            mxe = int(max_edges)
            max_edges = [mxe for _ in range(len(network_files))]
    else:
        max_edges = [None for _ in range(len(network_files))]
    if (network_weights is None) and (gs_network_file is not None):
        if annotation_file is None:
            ...
            return False
        network_weights = find_tp_weights(annotation_file, gs_network_file,
                                         network_files, network_names, wt_attr,
                                         max_edges, reverse_order, tp_out_file)
    print("""
       PARSED : network_names   : %s
       PARSED/COMPUTED : network_weights : %s
       MAX EDGES : max_edges : %s """ % (str(network_names),
                                         str(network_weights), str(max_edges)))
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("-x", "--max_edges", type=str,
    ...
    PARSER.add_argument("-p", "--tp_out_file",
                        type=argparse.FileType(mode='w'), required=False,
                        help="tp output file in tab-seperated format")
    ...
    print("""
       ...
       ARG : tp_out_file : %s """ %
          (str(ARGS.network_files), str(ARGS.network_names), str(ARGS.network_weights),
           ...
           str(ARGS.out_file), str(ARGS.tp_out_file)))
    if not main(ARGS.network_files, ARGS.network_names, ARGS.network_weights,
                ARGS.wt_attr, ARGS.max_edges, ARGS.max_out_edges,
                ARGS.max_avg, ARGS.reverse_order, ARGS.annotation_file,
                ARGS.gs_network_file, ARGS.out_file, ARGS.tp_out_file):
        PARSER.print_usage()
