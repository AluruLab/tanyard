...
from data_utils import load_reveng_network, load_annotation, map_probes, load_gsnetwork
...
def main(annot_file: str, gs_file: str, network_files: List[str], 
    ...
    if out_file:
        ...
        if len(network_files) == 3:
           fig, ax = venn.venn3(venn_labels, names=network_names)
           fig.savefig(out_file)
        if len(network_files) == 2:
           fig, ax = venn.venn2(venn_labels, names=network_names)
           fig.savefig(out_file)
    return True
    return True
...
if __name__ == "__main__":
    ...
    if not main(CMDARGS.annotation_file, CMDARGS.gs_network_file, CMDARGS.network_files, 
                CMDARGS.network_names, CMDARGS.dist, CMDARGS.out_file):
